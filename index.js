...
let getCountTable = function getCountTable (a, b) {
  ...
  keys.sort((a, b) => {
    ...
    else if (a.codePointAt(0) < b.codePointAt(0)) return -1
    return 0
  })
  for (let i of keys) {
    table[i] = [(a[i] || 0), (b[i] || 0)]
...
Object.defineProperty(window, 'dp', {
  enumerable: true,
  get: function () {
    return window.devicePixelRatio * dispScale
  }
})
let checkValidity
let resizeCanvas = function () {
  let rect = canv.getBoundingClientRect()
  canv.width = rect.width * window.dp
  canv.height = rect.height * window.dp
  checkValidity()
}
...
let updateCanvas = function () {
  ...
  ctx.font = (dp * 16) + 'px Fira Mono, Menlo, Monaco, Inconsolata, Consolas, Lucida Console, monospace'
  ctx.textAlign = 'center'
  let height = Math.min(canv.height, dispHeight * dp)
  let index = 0
  let chars = {}
